...
export function buildContentSecurityPolicy(nonce: string, nodeEnv = process.env.NODE_ENV): string {
  ...
  return [
    ...
    "form-action 'self'",
    `script-src 'self' 'nonce-${nonce}'${developmentScriptPolicy} https://connect.facebook.net https://www.googletagmanager.com`,
    `style-src 'self' 'nonce-${nonce}'${developmentStylePolicy} https://fonts.googleapis.com`,
    ...
    "connect-src 'self' https://*.supabase.co https://api.openai.com https://api.anthropic.com https://graph.facebook.com https://accept.paymob.com https://*.paymob.com https://connect.facebook.net https://www.facebook.com https://web.facebook.com https://*.sentry.io https://*.ingest.sentry.io https://*.ingest.us.sentry.io",
